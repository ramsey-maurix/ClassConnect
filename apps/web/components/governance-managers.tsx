...
export function LiveAdminAttendance() {
  ...
  return <div className="stack">
    ...
    <Card className="table-shell">
      <PaginatedTable><table><thead><tr><th>Session</th><th>Programme / class</th><th>Lecturer</th><th>Date</th><th>Method</th><th>Present</th><th>Late</th><th>Absent</th><th>Status</th><th>Details</th></tr></thead><tbody>
...
export function LiveAdminAudit() {
  ...
  return <><PageHeader title="Audit Log" description="Live immutable record of sensitive system actions." /><Card className="table-shell"><PaginatedTable><table><thead><tr><th>Date</th><th>Actor</th><th>Action</th><th>Record</th><th>Description</th><th>IP address</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{new Date(item.createdAt).toLocaleString()}</td><td>{item.actor ? `${item.actor.firstName} ${item.actor.lastName}` : "System"}</td><td><Badge tone="info">{item.action.replaceAll("_", " ")}</Badge></td><td>{item.entityType}{item.entityId ? ` · ${item.entityId.slice(0, 8)}` : ""}</td><td>{item.description}{item.reason ? <><br /><span style={{ color: "var(--muted)" }}>Reason: {item.reason}</span></> : null}</td><td>{item.ipAddress ?? "—"}</td></tr>)}</tbody></table></PaginatedTable></Card></>;
}
