...
export function LiveGradeBook() {
  ...
  async function createAssessment(event: FormEvent) {
    ...
    try { const created = await apiRequest<Assessment>("/assessments", { method: "POST", body: JSON.stringify({ courseId, title: title.trim(), type, maximumMark: parsedMaximum, weight: parsedWeight }) }); setCreating(false); await load(courseId); setAssessmentId(created.id); setMarks(Object.fromEntries(students.map((student) => [student.id, 0]))); toast("Assessment created", "All student marks start at 0 and can now be entered.", "success"); } catch (error) { toast("Assessment could not be created", message(error), "danger"); }
  }
